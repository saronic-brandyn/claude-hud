...
const defaultDeps: FileCacheDeps = {
  random: Math.random,
  now: Date.now,
};

/** Cache directory for a given home dir. Exported for tests. */
export function getCacheDir(homeDir: string): string {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (isTmp) {
          if (age > MAX_TMP_AGE_MS) fs.unlinkSync(fullPath);
          continue;
        }
...
export class FileCache<T> {
  ...
  private readonly deps: FileCacheDeps;
